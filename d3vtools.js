  ...
  <script>
    ...
    function treeRows(entries) {
      return (entries || []).map((e) => {
        ...
        return `
          <div class="tree-item" data-id="${esc(e.id)}" style="${active ? "outline:1px solid rgba(123,163,255,.5)" : ""}">
            <div class="tree-left">
            ...
            <div class="item-top">
              <div style="min-width:0;flex:1">
                ...
                <div class="muted" style="margin-top:6px">
                  ${esc(String(r.status || ""))} · ${esc(String(r.size || 0))} bytes · depth ${esc(String(r.depth ?? 0))}
                </div>
              </div>
              <div class="resource-btns">
  ...
  while (queue.length && entries.length < CONFIG.MAX_ASSETS + CONFIG.MAX_PAGES) {
    ...
    if (type.kind === "html") {
      ...
      if (job.depth + 1 <= maxDepth) {
        // keep crawling discovered same-origin pages
