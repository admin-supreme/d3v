...
<body>
  <div class="shell">
    <header class="hero">
      <div class="hero-copy">
        <p class="eyebrow">𝓑𝔂 𝑳.𝑨𝒓𝒂𝒔𝒉𝒊</p>
        <h1>𝑾𝒆𝒃 𝑨𝒏𝒂𝒍𝒚𝒔𝒊𝒔 𝑷𝒓𝒐𝒕𝒐𝒄𝒐𝒍</h1>
        <p class="sub">
          𝚃𝚑𝚒𝚜 𝚒𝚜 𝚊𝚗 𝙰𝙿𝙸 𝚋𝚊𝚜𝚎𝚍 𝙰𝚗𝚊𝚕𝚢𝚝𝚒𝚌𝚊𝚕 𝙴𝚗𝚐𝚒𝚗𝚎 𝙳𝚎𝚜𝚒𝚐𝚗𝚎𝚍 𝚋𝚢 𝚝𝚘 𝚎𝚡𝚝𝚛𝚊𝚌𝚝 𝚖𝚘𝚜𝚝 𝚊𝚌𝚌𝚞𝚛𝚊𝚝𝚎 𝚍𝚊𝚝𝚊 𝚜𝚝𝚛𝚞𝚌𝚝𝚞𝚛𝚎 𝚏𝚛𝚘𝚖 𝚊𝚗𝚢 𝙷𝚃𝚃𝙿/𝙷𝚃𝚃𝙿𝚂 𝚜𝚒𝚝𝚎. 𝙱𝚎𝚜𝚝 𝚊𝚕𝚝𝚎𝚛𝚗𝚊𝚝𝚒𝚟𝚎 𝚘𝚏 𝙳𝚎𝚟𝚃𝚘𝚘𝚕𝚜
          </p>
      </div>
...
function extractFromHtml(html, baseUrl, rootOrigin) {
  ...
  const styleSelectors = [
    /<link\b[^>]*rel=["'][^"']*\bstylesheet\b[^"']*["'][^>]*href=["']([^"']+)["'][^>]*>/gi,
    /<link\b[^>]*href=["']([^"']+\.css(?:[?#][^"']*)?)["'][^>]*>/gi,
  ];
  ...
  for (const re of pageSelectors) {
    let m;
    while ((m = re.exec(htmlText)) !== null) {
      const abs = canonicalizeSameOrigin(m[1], baseUrl, rootOrigin);
      if (!abs) continue;
  ...
  for (const re of styleSelectors) {
    let m;
    while ((m = re.exec(htmlText)) !== null) {
      const abs = canonicalizeSameOrigin(m[1], baseUrl, rootOrigin);
      if (!abs) continue;
  ...
  for (const re of scriptSelectors) {
    let m;
    while ((m = re.exec(htmlText)) !== null) {
      const abs = canonicalizeSameOrigin(m[1], baseUrl, rootOrigin);
      if (!abs) continue;
  ...
  for (const re of assetSelectors) {
    let m;
    while ((m = re.exec(htmlText)) !== null) {
      const abs = canonicalizeSameOrigin(m[1], baseUrl, rootOrigin);
      if (!abs) continue;
  ...
  for (const css of inlineStyles) {
    ...
    out.assets.push(...asArray(extracted.assets));
  }
  out.inlineBlocks = extractInlineBlocks(htmlText);
  return {
    pages: unique(out.pages),
    styles: unique(out.styles),
    scripts: unique(out.scripts),
    links: unique(out.links),
    assets: unique(out.assets),
    inlineBlocks: asArray(out.inlineBlocks),
  };
}
