...
function terminalPathLabel(path) {
    ...
    function buildTerminalTree(entries, rootUrl) {
      const siteLabel = (() => {
        try {
...
function renderTerminalTreePanel() {
      const d = state.data;
      const target = $("#terminalTree");
      if (!target) return;
      if (!d) {
        target.textContent = "No site loaded yet.";
        return;
      }
      target.textContent = d.treeText || buildTerminalTree(d.entries || [], d.rootUrl);
    }
    ...
    $("#copyTreeText").addEventListener("click", () => {
      const d = state.data;
      const text = d ? (d.treeText || buildTerminalTree(d.entries || [], d.rootUrl)) : "No site loaded yet.";
      copyText(text);
    });
