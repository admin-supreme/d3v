...
function fitTerminalTree() {
  const pre = $("#terminalTree");
  if (!pre) return;
  ...
  const cap = Math.max(180, Math.floor(window.innerHeight * 0.55));
  pre.style.height = `${Math.min(pre.scrollHeight, cap)}px`;
}
...
function resolveCurrentOriginalUrl() {
  const f = frame();
  if (f?.dataset?.originalUrl) return f.dataset.originalUrl;
...
function setIframeUrl(url, html = "") {
  const iframe = frame();
  if (!iframe) return;
  if (html) {
  ...
  delete iframe.dataset.originalUrl;
  iframe.src = url;
  setLiveUrl(url || "");
}
